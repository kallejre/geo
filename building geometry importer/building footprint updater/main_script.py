...
# Nii. Praeguseks reaks on api päring tehtud ning saadud vastu json-fail.
######################################################################################
...
with open(overlap_csv_fname, 'w', newline='', encoding='utf8') as csvfile:
    ...
    csvwriter.writerow('Kattuvuse %|Keskpunktide kaugus (m)|Ads-check|SHP-ID|OSM-ID'.split('|')+['etak OSM', 'street','housenumber', 'housename', 'etak MA', 'ads_lahiaa'])
    for yhikruut in overlapping:
        for way_id in overlapping[yhikruut]:
            for item in overlapping[yhikruut][way_id]:
                try:
                    csvwriter.writerow(list(map(lambda x:str(x).replace('.',','), item[:2]))+[str(item[2]).upper()]+list(item[3:]))
