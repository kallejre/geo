# Funktsioonid, mida skript vajab
...
def overpass_wrapper(search_Area, aastaid=10, exclude_shared_nodes=False, update=False, JSON=True):
    ...
    if type(search_Area) in {list, tuple}:
        ...
        if '/' in search_Area and search_Area.split('/')[0] in {'way','relation'}:
            ...
            search_Area='area:'+str(int(search_Area.split('/')[1])+ (2400000000 if search_Area.split('/')[0]=='way' else 3600000000))
        else:
            areacode=False
    else:
...
def lest_geo(y,x):
    ...
    u = 1.5707963268 - (2.0 * math.atan((((yy **2 + xx **2)**0.50000000000) / 11473298.0383961)**1.1707191095724578))
    LAT = (u + 0.0033565514853524606 * math.sin(2.0 * u) + (6.5718727752991235e-06) * math.sin(4.0 * u) +
     (1.7645643646802425e-08) * math.sin(6.0 * u) + (5.328478549061744e-11) * math.sin(8.0 * u))* 57.2957795130823
...
def update_geometry(sf,osm_id, shp_id, changes=0):
    ...
    for i in waydi:
        if i['type']=='node':
            ...
            if len(ways_through_node)!=1:
                fails.add(('FAIL', 'Hoone sõlm on jagatud teise hoonega.', tuple(map(lambda x: x['id'], ways_through_node))))
    ...
    for i in waydi:
        if i['type']=='node':
            ...
            lahim=min(geom, key=lambda node: basic_dist(node, (i['data']['lat'],  i['data']['lon']))+bool(node in kasutatud_shp))
    ...
    node_stats=list(map(str, [osm_id, len(osm_node_list)-1,news,round(news/(len(osm_node_list)-1-news),2)]))
    if not fails:
