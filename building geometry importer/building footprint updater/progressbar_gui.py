    ...
    def update(self, lvl, change=1, label=None):
        ...
        if self.timer:
            ...
            if self.prog_val[lvl] != 0:
                eta = round(
                    self.t_start[lvl] - t_cur + ((t_cur - self.t_start[lvl]) / self.prog_val[lvl] * self.max[lvl]))
            else:
                eta = float('inf')
            self.label_obj[lvl].config(
                text='{0} {1}/{2}  {3}s veel'.format(self.label_txt[lvl], self.prog_val[lvl], self.max[lvl], eta))
        else:
            self.label_obj[lvl].config(
                text='{0} {1}/{2}'.format(self.label_txt[lvl], self.prog_val[lvl], self.max[lvl]))
        self.aken.update()
