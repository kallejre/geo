...
function get_taglist(query){
	...
	tag=query.split("=")
	params={'page' : 1,
		'rp':10,'sortname':'count_ways','sortorder':'desc'}
	...
	if (tag.length==1){
		var out = response.data.sort(function(d){return !d.key.startsWith(params.query)}).map(function(d){return d.key})
	} else {
		var out = response.data.sort(function(d){return !d.value.startsWith(params.query)}).map(function(d){return d.value})
	}
